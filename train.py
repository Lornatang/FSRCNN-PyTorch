...
for epoch in range(args.epochs):
    ...
    # Save model
    if (epoch + 1) % 20 == 0:
        torch.save(model.state_dict(), f"weights/fsrcnn_{args.scale_factor}x_epoch_{epoch + 1}.pth")
    if avg_psnr > best_psnr:
        best_psnr = avg_psnr
        torch.save(model.state_dict(), f"weights/fsrcnn_{args.scale_factor}x.pth")
