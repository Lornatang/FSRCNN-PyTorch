...
class TrainValidImageDataset(Dataset):
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        if self.mode == "Train":
            # Data augment
            lr_y_image, hr_y_image = imgproc.random_crop(lr_y_image, hr_y_image, self.image_size, self.upscale_factor)
            lr_y_image, hr_y_image = imgproc.random_rotate(lr_y_image, hr_y_image, angles=[0, 90, 180, 270])
        elif self.mode == "Valid":
            lr_y_image, hr_y_image = imgproc.center_crop(lr_y_image, hr_y_image, self.image_size, self.upscale_factor)
        else:
    ...
    def read_image_to_memory(self) -> None:
        progress_bar = tqdm(self.image_file_names,
                            ...
                            unit="image",
                            desc=f"Read {self.mode.lower()} dataset into memory")
