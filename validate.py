...
def main() -> None:
    ...
    for index in range(total_files):
        ...
        # Open low-resolution images.
        lr_image = Image.open(lr_path).convert("RGB")
        bic_image = lr_image.resize([lr_image.width * upscale_factor, lr_image.height * upscale_factor], Image.BICUBIC)
        # Extract Y channel for low-res image data.
        ...
        # Extract Y channel for bicubic image data.
        bic_image = np.array(bic_image).astype(np.float32)
        bic_ycbcr = convert_rgb_to_ycbcr(bic_image)
